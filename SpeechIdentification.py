...
def train_model():
    ...
    for path in file_paths:
        ...
        if speaker_name not in speakers_features:
            speakers_features[speaker_name] = np.asarray(())

        sr, audio = read(source + path)
        vector = extract_features(audio, sr)
        
        if speakers_features[speaker_name].size == 0:
            speakers_features[speaker_name] = vector
        else:
            speakers_features[speaker_name] = np.vstack((speakers_features[speaker_name], vector))

    for speaker_name, features in speakers_features.items():
        if features.shape[0] > 0:
            gmm = GaussianMixture(n_components=6, max_iter=200, covariance_type='diag', n_init=3)
            gmm.fit(features)
            # Save the trained Gaussian model
            picklefile = speaker_name + ".gmm"
            with open(os.path.join(dest, picklefile), 'wb') as model_file:
...
def test_model():
    ...
    threshold = -30  # Example threshold, adjust based on your data and experiments
    ...
    for path in file_paths:
        ...
        if log_likelihood[winner] < threshold:
            print(path, "\tdetected as - Not Identified")
        else:
